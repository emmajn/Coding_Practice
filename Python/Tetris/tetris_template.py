...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        new_shape = random.choice([I_shape, J_shape, L_shape, O_shape, S_shape, T_shape, Z_shape])
    ...
    def key_pressed(self, event):
        ...
        #YOUR CODE HERE
        key = event.keysym
        if key in ['Down', 'Right', 'Left','space']:
            if key == 'Up':
                self.do_rotate()
            else:
...
win = GraphWin("Tetris",10 * Block.BLOCK_SIZE,20* Block.BLOCK_SIZE)
game = Tetris(win)
win.mainloop()
