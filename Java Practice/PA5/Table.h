...
class Table {
 public:
   ...
   // if key is not found, then this returns false and the hash table is unchanged
   bool changeScore(const std::string &key, int newValue);
 ...
 private:
   ...
   // ListType[hashSize] Hash table with hashSize number of bins
   ListType * tableData;
   
   // int: number of key/values in hash table
   int entriesCount; 
   
   // int: longest chain in hashTable
   int longestChain; 
   
   // int: Number of non-empty backets in hastable
   int nonEmptyBuckets; 
   
   // private function to see longest chain 
   void updateLongestChain();
